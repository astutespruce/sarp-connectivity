...
<div onkeydown={handleKeyDown} role="list">
	...
	{#if query.length > 0 && query.length < MIN_QUERY_LENGTH}
		<div class="italic text-muted-foreground">...keep typing...</div>
	...
	{:else if searchRequest.isSuccess}
		{#if results && results.length > 0}
			...
			{#if remaining > 0}
				<div class="text-sm my-4 italic text-center text-muted-foreground">
					...and {formatNumber(remaining)} more...
				</div>
			{/if}
		{:else}
			<div class="italic text-muted-foreground">No results match your search</div>
		{/if}
	{/if}
</div>
