...
units = ('states', 'HUC2', 'HUC4',
         'ecoregion1', 'ecoregion2', 'ecoregion3', 'ecoregion4')  # TODO: 'HUC8',
...
for unit in units:
    ...
    df = gp.read_file('data/src/sarp_{}.shp'.format(unit))
    df['bnd'] = df.apply(lambda row: row.geometry.envelope, axis=1)

    # Make sure that bounds are in WGS84
    df = df.set_geometry('bnd')[[field, 'bnd']].dissolve(
        by=field).to_crs({'init': 'EPSG:4326'})
