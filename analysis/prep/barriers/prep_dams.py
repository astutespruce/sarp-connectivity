...
# (0=not reviewed, 7,9 = assumed offstream but attempt to snap, 11 = duplicate, probably from a prior run of this)
snapped_df = snapped_df.loc[
    snapped_df.SARPID.notnull()
    & snapped_df.geometry.notnull()
    & (~snapped_df.ManualReview.isin([0, 7, 9, 11]))
].set_index("SARPID")
...
ix = df.ManualReview_snap.isin([4, 5, 15])
