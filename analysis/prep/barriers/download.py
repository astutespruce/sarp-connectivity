...
from analysis.constants import SARP_STATES, RECON_TO_FEASIBILITY, DAM_COLS, CRS
...
for state, url in DAM_URLS.items():
    ...
    print("Downloaded {:,} dams in {:.2f}".format(len(df), time() - download_start))
...
df = download_fs(SNAPPED_URL, fields=["AnalysisID", "SNAP2018"], token=token)

print("Projecting manually snapped dams...")
df = df.loc[df.geometry.notnull()].to_crs(CRS)

print("Downloaded {:,} snapped dams in {:.2f}".format(len(df), time() - download_start))
