...
for unit in units:
    ...
    spp_df = merge_lines(
        dissolve(
            df.loc[df[unit].isin([1, 2]), ["ReachCode", "geometry"]].explode(
                ignore_index=True
            ),
            by="ReachCode",
...
for unit in units:
    ...
    # DEBUG:
    write_dataframe(
        spp_df.drop(columns=["buf"]).reset_index(), f"/tmp/{unit}_lines.fgb"
    )
    ...
    pairs["upstream_dist"] = shapely.distance(
        pairs.nhd_upstream_pt, pairs.spp_line.values
    )
