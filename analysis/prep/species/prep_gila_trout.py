...
layer = "Lines_Gila_Trout_UnDissolved"
...
df = read_dataframe(infilename, layer=layer, force_2d=True, columns=[]).to_crs(CRS)

# aggregate back together
df["group"] = 1
df = merge_lines(df.explode(ignore_index=True), by="group").explode(ignore_index=True)
...
#     "/tmp/gila_trout_habitat_source_buffer.fgb",
# )
...
#     "/tmp/gila_trout_high_overlap_keep_lines.fgb",
# )


flowlines["gila_trout_habitat"] = flowlines.index.isin(keep_line_ids)

print(
    f"species habitat: {shapely.length(df.geometry.values).sum() / 1000:,.1f} km; "
    f"extracted {flowlines.loc[flowlines.gila_trout_habitat, ['length']].values.sum() / 1000:,.1f} km from NHD"
)
