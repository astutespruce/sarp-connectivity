...
keep_line_ids = np.array([40000600079668, 40000600098890, 40000600098813, 40000600105452])


df = read_dataframe(infilename, layer=layer, use_arrow=True, columns=[]).to_crs(CRS)
...
write_dataframe(
    ...
    "/tmp/apache_trout_habitat_source_buffer.fgb",
)
...
#     "/tmp/apache_trout_high_overlap_keep_lines.fgb",
# )


flowlines["apache_trout_habitat"] = flowlines.index.isin(keep_line_ids)

print(
    f"species habitat: {shapely.length(df.geometry.values).sum() / 1000:,.1f} km; "
    f"extracted {flowlines.loc[flowlines.apache_trout_habitat, ['length']].values.sum() / 1000:,.1f} km from NHD"
)
